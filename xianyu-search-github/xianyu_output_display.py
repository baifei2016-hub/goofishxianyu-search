...
# -*- coding: utf-8 -*-
"""
闲鱼输出显示版 - 保持所有功能不变，优化结果显示
功能：在对话框中显示完整的搜索结果，包括产品列表、价格、地区、链接
保持：所有原有功能 + 个人闲置功能
"""
...
def main():
    ...
    if len(sys.argv) < 5 or sys.argv[1] != '闲鱼搜':
        print("❌ 格式: python3 xianyu_output_display.py 闲鱼搜 关键词 最低价 最高价 [个人]")
    ...
    try:
        ...
        screenshot = f"output{personal_suffix}_{keyword}_min{min_price}_max{max_price}_{timestamp}.png"
        ...
        print(f"✅ 匹配成功: {linked_count}/{len(items)} 个产品有链接")
        ...
        if has_personal:
            print(f"筛选条件: 个人闲置 ✓")
        print(f"抓取数量: {len(sorted_items)} 个产品")
        print(f"链接数量: {linked_count} 个可点击链接")
        print("=" * 70)
        ...
        for i, item in enumerate(sorted_items[:15], 1):
            ...
            if item['link']:
                print(f"   🔗 链接: {format_link_for_chat(item['link'])}")
            else:
                print(f"   🔗 链接: 无链接")
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        personal_suffix = "_个人" if has_personal else ""
        filename = f"output{personal_suffix}_{keyword}_min{min_price}_max{max_price}_{timestamp}.json"
        ...
        print(f"   2. 价格范围: ¥{min_price}-¥{max_price}")
        print(f"   3. 个人闲置筛选: {'已应用 ✓' if has_personal else '未应用'}")
        print(f"   4. 抓取产品数: {len(sorted_items)} 个")
        print(f"   5. 链接匹配: {linked_count} 个可点击链接")
        print("=" * 70)
